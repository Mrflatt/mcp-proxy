package auth

import (
	"context"
	"net/http"
	"sync"

	"github.com/Mrflatt/mcp-proxy/config"
	"golang.org/x/oauth2"
	"golang.org/x/oauth2/google"
	"google.golang.org/api/impersonate"
)

type googleAccessTokenHandler struct {
	cfg *config.AuthConfig
	mu  sync.Mutex
	ts  oauth2.TokenSource
}

func newGoogleAccessToken(cfg *config.AuthConfig) *googleAccessTokenHandler {
	return &googleAccessTokenHandler{cfg: cfg}
}

// TokenSource returns the cached token source, creating it on first call or
// after an Authorize-triggered reset. The source itself handles token refresh.
func (h *googleAccessTokenHandler) TokenSource(ctx context.Context) (oauth2.TokenSource, error) {
	h.mu.Lock()
	defer h.mu.Unlock()
	if h.ts != nil {
		return h.ts, nil
	}

	var (
		ts  oauth2.TokenSource
		err error
	)
	if h.cfg.ServiceAccount != "" {
		ts, err = impersonate.CredentialsTokenSource(ctx, impersonate.CredentialsConfig{
			TargetPrincipal: h.cfg.ServiceAccount,
			Scopes:          h.cfg.Scopes,
		})
	} else {
		ts, err = google.DefaultTokenSource(ctx, h.cfg.Scopes...)
	}
	if err != nil {
		return nil, err // not cached — caller can retry after fixing credentials
	}
	h.ts = ts
	return ts, nil
}

// Authorize is called by the transport on 401/403. It invalidates the cached
// token source so the next request re-reads ADC credentials from disk.
func (h *googleAccessTokenHandler) Authorize(_ context.Context, _ *http.Request, resp *http.Response) error {
	resp.Body.Close()
	h.Reset()
	return nil
}

// Reset clears the cached token source so it is re-created from current ADC
// credentials on the next call.
func (h *googleAccessTokenHandler) Reset() {
	h.mu.Lock()
	h.ts = nil
	h.mu.Unlock()
}
